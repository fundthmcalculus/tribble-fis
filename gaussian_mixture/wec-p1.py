...
def load_data(idx: int = 1):
    ...
    y = X[f"Power{idx}"]
    y.name = "y_value"
    X.drop(columns=[col for col in X.columns if col != f"X{idx}" and col != f"Y{idx}"], inplace=True)
...
def main():
    ...
    if b_optimize_coeff:
        y_bucket_mean_opt_0 = optimize_tsk_coefficients(
            X_train, gaussian_memberships, top_n_todo, y_bucket_mean, y_train, order="0th"
        )
