...
| **A2** | **Fuzzy sequence + fuzzy syntax + fuzzy decoder** — next-token prediction over named semantic *and* syntactic dimensions, then Zadeh linguistic approximation | built; rules at logreg parity (bal-acc 0.528 → **0.569**), category-correct decoding; **as an LM: complementary to a bigram — mixing beats the bigram alone (228 vs 237)**; loses head-to-head; generation not yet grammatical | [`fuzzyembed/README.md`](fuzzyembed/README.md) |
...
| **A4** | **Fuzzy tokenizer + linguistic parameter space** — hybrid vocabulary (547 nameable units), graded segmentation, 66 named UD/shape/semantic/affect dims | built, wired into the ranker, **measured — and it lost**: 360.0 vs 324.8, and optimal mixture weight zero against every alternative (strictly dominated). Better *reporting* language, worse *predictive* one | [`fuzzytok/`](fuzzytok/) |
| **A5** | **Size/quality frontier** — how small the rule base can get on local compute | measured: **285 learned parameters reach ppl 341.2**, within 5% of the 860-rule model, vs 34,021 stored counts for a bigram at 264.2; rule count drives *inference* cost, not training cost | [`LOG.md` E22.3](LOG.md) |
| **A6** | **First-order TSK — word-level consequents** (fuzzy class-based LM): each rule carries a *word distribution*, not a scalar | **best result in the project: ppl 219.9 standalone, beating a bigram (286.4) and tuned trigram (284.4) head-to-head by 23%**; mixed with a trigram 201.7 at fuzzy weight 0.8, bigram weight 0. Generation category-ppl 10.10 vs real text 8.18, bigram 12.84 | [`fuzzyembed/firstorder.py`](fuzzyembed/firstorder.py) |
