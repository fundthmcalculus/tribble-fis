...
| **A6** | **First-order TSK — word-level consequents** (fuzzy class-based LM): each rule carries a *word distribution*, not a scalar | **best result: mixed with a bigram, ppl 256.2 vs 286.4 alone (−10.5%, λ=0.4)** on 1M tokens; standalone 343.5, beating the zero-order 355.5 and losing to the bigram | [`fuzzyembed/firstorder.py`](fuzzyembed/firstorder.py) |
